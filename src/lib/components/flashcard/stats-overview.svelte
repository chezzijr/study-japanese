<script lang="ts">
	...
	let { stats, showForecast = false, forecastData = [] }: {
		stats: DeckStats;
	...
	const maxForecastCount = $derived(
		forecastData.length > 0
			? Math.max(...forecastData.map(d => d.count), 1)
			: 1
	);
</script>
...
<div class="space-y-6">
	...
	<div class="card bg-base-200">
		<div class="card-body">
			...
			<div class="mt-4">
				<div class="text-sm text-base-content/60 mb-2">Tổng: {stats.totalCards} thẻ</div>
				{#if stats.totalCards > 0}
					...
					<div class="text-xs text-base-content/50 mt-1">
						{Math.round(((stats.totalCards - stats.newCards - stats.suspendedCards) / stats.totalCards) * 100)}% đã học ít nhất 1 lần
					</div>
				{/if}
			</div>
		</div>
	...
	<div class="card bg-base-200">
		<div class="card-body">
			...
			<div class="flex items-center gap-4">
				<div class="radial-progress text-primary" style="--value:{Math.min(100, (stats.averageEaseFactor / 4) * 100)}; --size:4rem;">
					...
					{#each forecastData as day}
						...
						>
							<div
								...
								style="height: {(day.count / maxForecastCount) * 100}%; min-height: {day.count > 0 ? '4px' : '0'}"
							></div>
