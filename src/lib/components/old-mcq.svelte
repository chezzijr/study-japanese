<script lang="ts">
	...
	let { quesList, ansList, hintList, keyName, valueName, swappable } = $props();
	...
	function nextQuestion() {
		if (swp) quesType = Math.random() > 0.5 ? 'first' : 'second';
...
<main class="flex h-[95vh] w-screen flex-col items-center justify-center gap-10">
	{#if swappable}
		<label class="swap swap-flip text-9xl">
			...
			<div class="swap-off tooltip" data-tip="Hỏi 1 chiều">😇</div>
			<div class="swap-on tooltip" data-tip="Hỏi 2 chiều">😈</div>
		</label>
	{/if}
	<h1 class="h1">{questionTitle()}</h1>
	{#if hintList && hintList[index]}
		<button id="hint" class="btn btn-outline btn-primary" onclick={() => (showHint = !showHint)}
			>Hint</button
		>
		{#if showHint}
	...
	<section class="grid w-[60%] grid-cols-2 gap-4">
		...
		>
		<button class="btn btn-outline btn-success" onclick={(e) => handleClick(e, 1)}
			>{options[1]}</button
		>
		<button class="btn btn-outline btn-warning" onclick={(e) => handleClick(e, 2)}
			>{options[2]}</button
		>
		<button class="btn btn-outline btn-error" onclick={(e) => handleClick(e, 3)}
			>{options[3]}</button
		>
	</section>
</main>
